...
Mat edge_nms(Mat &E, Mat &O, int r, int s, float m, int nThreads) {
    Mat E_t, O_t, E_nms, E_nms_t;
    transpose(E, E_t);
    transpose(O, O_t);

    E_nms_t = Mat(E.cols, E.rows, E.type());
    mexFunction((float *)E_t.data, (float *)O_t.data, (float *)E_nms_t.data, r, s, m, nThreads, E.rows, E.cols);
    transpose(E_nms_t, E_nms);
