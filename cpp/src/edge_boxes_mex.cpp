...
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  ...
  if(mxGetClassID(pr[0])!=mxUINT8_CLASS) mexErrMsgTxt("Image must be uint8 class.");
  if(mxGetClassID(pr[1])!=mxCHAR_CLASS) mexErrMsgTxt("Model path must be a string.");

  char * str = mxArrayToString(pr[1]);
  ...
  uint8* img_rgb = (uint8*) mxGetPr(pr[0]);
  int* dims = (int*) mxGetDimensions(pr[0]);
  int height = dims[0];
  int width = dims[1];
  int area = width * height;

  // convert to opencv BGR format
  cv::Mat imR(1, area, cv::DataType<uint8>::type, img_rgb);
  cv::Mat imG(1, area, cv::DataType<uint8>::type, img_rgb + area);
...
//  TODO: Takes 200ms
  ...
  myfile << format(im_edge,"csv") << std::endl << std::endl;
  myfile.close();
#endif
